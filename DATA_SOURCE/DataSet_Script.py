...
def introduce_random_missing_values(df, missing_percentage=0.1):
    ...
    num_missing = int(missing_percentage * total_entries)  # Calculate total number of missing values
    
    for _ in range(num_missing):
        # Randomly select a row and column to introduce a missing value
...
# Define numerical columns for normalization and missingness
...
print(f"Introduced missing values at {0.1*100}% rate.")

# Feature normalization using MinMaxScaler
scaler = MinMaxScaler()
df[numerical_columns] = scaler.fit_transform(df[numerical_columns])

# Optionally, you can choose to save the data before handling missing values
# df.to_csv('Traffic_Data_With_Missing.csv', index=False)

# Save the DataFrame to CSV
df.to_csv('Traffic_Data_3.csv', index=False)

print("Traffic data generation with missing values and normalization complete!")
